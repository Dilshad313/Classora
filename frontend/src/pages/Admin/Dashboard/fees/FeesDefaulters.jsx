...
const FeesDefaulters = () => {
  ...
  const [viewMode, setViewMode] = useState('table'); // 'table' or 'large'
  const [selectedDefaulters, setSelectedDefaulters] = useState([]);

  // Sample defaulters data
  const defaulters = [
    {
      id: 1,
      registrationNo: 'REG001',
      photo: 'https://ui-avatars.com/api/?name=John+Doe&background=4F46E5&color=fff',
      name: 'John Doe',
      guardian: 'Robert Doe',
      class: 'Class 10-A',
      payable: 5000,
    ...
    },
    {
      ...
      photo: 'https://ui-avatars.com/api/?name=Jane+Smith&background=7C3AED&color=fff',
      name: 'Jane Smith',
    ...
    },
    {
      ...
      photo: 'https://ui-avatars.com/api/?name=Mike+Johnson&background=EC4899&color=fff',
      name: 'Mike Johnson',
    ...
    },
    {
      ...
      photo: 'https://ui-avatars.com/api/?name=Sarah+Williams&background=F59E0B&color=fff',
      name: 'Sarah Williams',
    ...
    },
    {
      ...
      photo: 'https://ui-avatars.com/api/?name=Emily+Brown&background=10B981&color=fff',
      name: 'Emily Brown',
      guardian: 'William Brown',
      class: 'Class 11-A',
      payable: 6000,
    ...
    },
    {
      ...
      photo: 'https://ui-avatars.com/api/?name=Alex+Taylor&background=3B82F6&color=fff',
      name: 'Alex Taylor',
      guardian: 'Richard Taylor',
      class: 'Class 9-A',
      payable: 3800,
  ...
  const filteredDefaulters = defaulters.filter(defaulter => {
    ...
    return (
      ...
      defaulter.guardian.toLowerCase().includes(query) ||
  ...
  // Handle submit (navigate to Collect Fees)
  const handleSubmit = () => {
    if (selectedDefaulters.length === 0) {
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-6 no-print">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-end">
              <button
                onClick={() => feesMonth && alert(`Loading defaulters for ${feesMonth}`)}
                className="px-6 py-3 bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white rounded-xl font-semibold shadow-md hover:shadow-lg transition-all"
              >
                Load Defaulters
              </button>
            </div>
                    ...
                    <tr key={defaulter.id} className="hover:bg-red-50 transition-colors">
                      <td className="px-4 py-4">
                        <input
                          ...
                          onChange={() => handleSelectDefaulter(defaulter.id)}
                          className="w-4 h-4 rounded border-gray-300"
                        />
                      </td>
                      <td className="px-4 py-4 text-gray-700 font-medium">{defaulter.id}</td>
                      <td className="px-4 py-4">
                        <img
                          src={defaulter.photo}
                          alt={defaulter.name}
                      ...
                      <td className="px-4 py-4">
                        <div className="font-semibold text-gray-900">{defaulter.name}</div>
                        <div className="text-sm text-gray-600">{defaulter.registrationNo}</div>
                      </td>
                      <td className="px-4 py-4 text-gray-700">{defaulter.guardian}</td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4 text-right">
                        <span className="text-red-600 font-bold text-lg">₹{defaulter.payable.toFixed(2)}</span>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center justify-center gap-2">
                          ...
                          <button
                            onClick={() => alert(`Sending reminder to ${defaulter.name}`)}
                    ...
                    <tr key={defaulter.id} className="hover:bg-red-50 transition-colors">
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          />
                          <div>
                            <div className="font-bold text-gray-900 text-lg">{defaulter.name}</div>
                            <div className="text-sm text-gray-600">Guardian: {defaulter.guardian}</div>
                          </div>
                        </div>
                      </td>
                    </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center p-4 bg-green-50 rounded-xl border border-green-200">
              <div className="text-3xl font-bold text-green-600">
                ₹{filteredDefaulters.reduce((sum, d) => sum + d.payable, 0).toFixed(2)}
              </div>

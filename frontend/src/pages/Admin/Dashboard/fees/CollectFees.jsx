        ...
        <div className="mb-8 no-print">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-gray-600 mt-1">Collect and manage student fee payments</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 mb-6 no-print">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {['student', 'family'].map(type => (
              ...
              >
                {type === 'student' ? 
                  <User className={`w-8 h-8 mx-auto mb-3 ${collectionType === type ? 'text-green-600' : 'text-gray-400'}`} /> : 
                  <Users className={`w-8 h-8 mx-auto mb-3 ${collectionType === type ? 'text-green-600' : 'text-gray-400'}`} />
                }
                <h3 className={`font-bold text-lg ${collectionType === type ? 'text-green-600' : 'text-gray-700'}`}>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-6 no-print">
          ...
          {selectedEntity && (
            <div className="mt-4 p-4 bg-green-50 border-2 border-green-200 rounded-xl flex items-center gap-2">
              ...
              <span className="text-green-800 font-semibold">Selected: {selectedEntity.studentName}</span>
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mb-6 no-print">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 p-6 bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200">
              <div>
                ...
                <div className="font-bold text-gray-900">{selectedEntity.registrationNo}</div>
              </div>
              <div>
                <div className="flex items-center gap-2 text-sm text-gray-600 mb-1">
                  <User className="w-4 h-4" />Student Name
                </div>
                <div className="font-bold text-gray-900">{selectedEntity.studentName}</div>
              </div>
              <div>
                <div className="flex items-center gap-2 text-sm text-gray-600 mb-1">
                  <UserCircle className="w-4 h-4" />Guardian Name
                </div>
                <div className="font-bold text-gray-900">{selectedEntity.guardianName}</div>
              </div>
              <div>
                <div className="flex items-center gap-2 text-sm text-gray-600 mb-1">
                  <GraduationCap className="w-4 h-4" />Class
                </div>
                <div className="font-bold text-gray-900">{selectedEntity.class}</div>
              </div>
            </div>
              ...
              <div className="overflow-x-auto rounded-xl border-2 border-gray-200">
                <table className="w-full">
                  ...
                  <tfoot className="bg-green-100 border-t-2 border-green-300">
                    <tr>
                      <td colSpan="2" className="px-4 py-4 text-left font-bold text-gray-900 text-lg">Total Amount</td>
                      <td className="px-4 py-4 text-right font-bold text-green-600 text-xl">₹{totalAmount.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              ...
              <div>
                ...
                <div className="px-4 py-3 bg-red-50 border-2 border-red-200 rounded-xl">
                  <span className="text-2xl font-bold text-red-600">₹{selectedEntity.dueBalance?.toFixed(2) || '0.00'}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border-2 border-gray-300 p-12 print-section">
              ...
              <div className="grid grid-cols-2 gap-6 mb-8">
                <div>
                  ...
                  <p className="text-lg font-bold text-gray-900">{receiptData.receiptNo}</p>
                </div>
              ...
              <div className="mb-8 p-6 bg-gray-50 rounded-xl border border-gray-200">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">{receiptData.entity.registrationNo}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Student Name:</p>
                    <p className="font-semibold text-gray-900">{receiptData.entity.studentName}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Guardian Name:</p>
                    <p className="font-semibold text-gray-900">{receiptData.entity.guardianName}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Class:</p>
                    <p className="font-semibold text-gray-900">{receiptData.entity.class}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">Fee Month:</p>
                ...
                <table className="w-full border-2 border-gray-300">
                  <thead>
                    <tr className="bg-green-600 text-white">
                      ...
                      <th className="border border-gray-300 px-4 py-3 text-left">Particulars</th>
                  ...
                  <tbody>
                    {receiptData.feeParticulars.map((fee, index) => (
                      <tr key={index}>
                        ...
                        <td className="border border-gray-300 px-4 py-3 font-semibold">{fee.particular}</td>
                        <td className="border border-gray-300 px-4 py-3 text-right font-semibold">₹{fee.amount.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="bg-green-100">
                      <td colSpan="2" className="border border-gray-300 px-4 py-4 font-bold text-lg">Total Amount Paid</td>
                      <td className="border border-gray-300 px-4 py-4 text-right font-bold text-xl text-green-600">
